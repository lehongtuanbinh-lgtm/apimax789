...
// --- CẤU HÌNH ---
const PORT = 3000;
const API_URL = "https://underwear-twice-advertisements-differences.trycloudflare.com/api/txmd5";
...
let lastSessionChangeTime = 0; // NÂNG CẤP VIP: Theo dõi thời gian chuyển phiên để tính toán delay 10s
...
function extractFeatures(history) {
    ...
    // --- NÂNG CẤP VIP: DEEP HEX ENTROPY ANALYSIS ---
    // Phân tích sâu mã Hex của tổng xí ngầu để nhận diện cầu chìm
    const hexStr = totals.map(t => t.toString(16)).join('');
    const hexFreq = {};
    for(let char of hexStr) hexFreq[char] = (hexFreq[char] || 0) + 1;
    let hexEntropy = 0;
    for(let k in hexFreq) {
        let p = hexFreq[k] / hexStr.length;
        hexEntropy -= p * Math.log2(p);
    }
    ...
    return {
        ...
        entropy: entropy(tx),
        hexEntropy: hexEntropy, // Dữ liệu phân tích Hex mới
...
// =====================================================================
// === THUẬT TOÁN 14 & 15 VÀ BỘ TỪ ĐIỂN THUẬT TOÁN GIỮ NGUYÊN 100% ===
// =====================================================================
...
// Thuật toán 21: Bẫy Phản Đề Phản Biện - Smart Anti-Reversal
...
// Thuật toán 22: Nhận Diện Cầu Nhảy 1-1 Cương Quyết (Perfect 1-1 Locking)
...
// Thuật toán 23: Nhận Diện Nhịp Cầu 2-2 Chốt Chuẩn (Strict 2-2 Locking Engine)
...
// Thuật toán 24: Bộ Lọc Động Năng Nến Xúc Xắc Kép (Dual Dice Kinetic Entropy)
...
// Thuật toán 25: Phổ Chu kỳ Sóng Fibonacci (Fibonacci Waves Analyzer)
...
// Thuật toán 26: Chuỗi Markov Inversion Đa Cấp (Multi-stage Inverted Markov)
...
// Thuật toán 27: Phân Tích Độ Biến Động Chu Kỳ Nén (Compression Variance Radar)
...
// Thuật toán 28: Bộ So Khớp Lịch Sử Chuỗi Sâu (Deep Sequence Matching V9.0)
...
// Thuật toán 29: Trọng Cực Biến Động MD5 (MD5 Gravity Extremes)
...
// Thuật toán 30: Vi Long Tuyệt Đối Khóa Chốt (Vi Long Absolute Overlord)
...
// --- DANH SÁCH THUẬT TOÁN ĐẦY ĐỦ (30 THUẬT TOÁN VIP) ---
const ALL_ALGS = [
...
class SEIUEnsemble {
    ...
    predict(history) {
        ...
        // --- NÂNG CẤP VIP DEEP HEX: BẮT NHỊP 1-1 TỨC THÌ (PRE-OVERRIDE) ---
        // Phân tích lịch sử: Vừa báo T-X hoặc X-T là nhảy nhịp luôn, bẻ gãy lối đánh đuôi
        const last3Tx = history.slice(-3).map(h => h.tx).join('');
        const last2Tx = history.slice(-2).map(h => h.tx).join('');
        
        if (last2Tx === 'TX' && last3Tx !== 'XTX') {
            return {
                prediction: 'Tài',
                confidence: 0.99,
                rawPrediction: 'T',
                note: "🔥 [DEEP HEX] BẮT NHỊP SỚM VÀO KHUÔN 1-1 (T-X -> T)"
            };
        }
        if (last2Tx === 'XT' && last3Tx !== 'TXT') {
            return {
                prediction: 'Xỉu',
                confidence: 0.99,
                rawPrediction: 'X',
                note: "🔥 [DEEP HEX] BẮT NHỊP SỚM VÀO KHUÔN 1-1 (X-T -> X)"
            };
        }

        // --- NHẬN DIỆN VÀ KHÓA CỨNG NHỊP CẦU TUYỆT ĐỐI (1-1 / 2-2) ---
        // Không sử dụng đảo ngược kết quả, chốt trực tiếp.
        ...
        for (const a of this.algs) {
            try {
                ...
                // Các thuật toán VIP được boost trọng số cực mạnh
        ...
        // Boost % dựa trên độ lệch Hex Entropy nếu có
        const hexBoost = features.hexEntropy < 0.25 ? 0.05 : 0;
        const confidence = Math.min(0.98, Math.max(0.50, baseConfidence + consensusBonus + hexBoost));
        ...
        // --- QUY TRÌNH CHỐT KẾT QUẢ TRỰC TIẾP (KHÔNG SỬ DỤNG ĐẢO KẾT QUẢ) ---
        if (isBet) {
...
class SEIUManager {
    ...
    loadInitial(lines) {
        ...
        lastSessionChangeTime = 0; // Đặt về 0 lúc boot để không bị delay ảo
    }
...
async function fetchAndProcessHistory() {
    ...
    try {
        ...
        if (!currentSessionId) {
            ...
            // CÓ PHIÊN MỚI -> Kích hoạt Cờ khóa Delay 10 Giây
            lastSessionChangeTime = Date.now();
            ...
            currentSessionId = lastSessionInHistory.session;
            if (newRecords.length > 0) console.log(`🆕 Cập nhật ${newRecords.length} phiên. Phiên cuối: ${currentSessionId}. ⏳ Bắt đầu phân tích Deep Hex 10s...`);
        }
    } catch (e) {
...
app.get("/api/taixiumd5/lc79", async () => {
    ...
    // NÂNG CẤP VIP: Delay 10 giây (Deep Analysis Simulator)
    // Tính toán thời gian kể từ khi có phiên mới
    const now = Date.now();
    const timeSinceNewSession = now - lastSessionChangeTime;
    const isAnalyzing = timeSinceNewSession < 10000; // Nhỏ hơn 10s (10000ms)

    if (isAnalyzing && lastSessionChangeTime !== 0) {
        return {
            id: "by VIP @cskhvilong",
            ...
            pattern: "⏳ Hệ thống Deep Hex đang phân lọc cầu (10s)...", // Thay đổi trạng thái pattern
            phien_hien_tai: lastResult.session + 1,
            du_doan: "ĐANG PHÂN TÍCH...", // Khóa kết quả
            do_tin_cay: "Đang tính mã Hex", // Khóa %
            no_random: "by vi long"
        };
    }

    // Nếu qua 10 giây (hoặc mới boot up), trả về kết quả bình thường
    return {
        id: "by VIP @cskhvilong",
        phien_truoc: lastResult.session,
        tong: lastResult.total,
...
app.get("/", async () => { 
    return {
        ...
        version: "9.2 AI Vi Long Absolute Core (Hex Deep Analysis Version)", 
        algorithms: ALL_ALGS.length,
        pattern_recognition: "ULTRA VIP (Khóa cứng 1-1 Sớm, Phân lọc 10s Deep Hex)",
        endpoints: [
            "/api/taixiumd5/lc79",
...
const start = async () => {
    ...
    console.log("\n🚀 AI Tài Xỉu MD5 Pro V9.2 - ULTRA GOD MODE Đã Khởi Động!");
    ...
    console.log("   • [Deep Hex Delay]: Khóa 10s chờ lọc cầu cực êm trước khi nhả kết quả.");
};
start();
